...
namespace Valve.VR
{
    public class SteamVR_Skeleton_Poser : MonoBehaviour
    {
        ...
        protected void Awake()
        {
            ...
            {
                var pose = GetPoseByIndex(i);
                if (pose == null)
                {
                    Debug.LogError("[SteamVR] Skeleton Poser: Pose " + i + " is null. Did you forget to set it?", this);
                    continue;
                }
                blendPoses[i] = new SkeletonBlendablePose(pose);
        ...
        public class SkeletonBlendablePose
        {
            ...
            {
                pose = p;
                if (p.rightHand == null || p.leftHand == null)
                {
                    Debug.LogError("[SteamVR] Skeleton Poser: Pose " + p.name + " is missing a hand.", p);
                    return;
                }
                if (p.rightHand.bonePositions == null || p.leftHand.bonePositions == null)
                {
                    Debug.LogError("[SteamVR] Skeleton Poser: Pose " + p.name + " is missing bone data.", p);
                    return;
                }
